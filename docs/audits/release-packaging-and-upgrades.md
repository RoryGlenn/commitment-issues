...
| `node --test test/release-metadata.test.mjs test/release-recovery.test.mjs`                  | Passed, 58/58. Future release metadata, fixed legacy exceptions, exact title/body recovery, retry states, provenance continuity, and source/digest/publication mismatches are mocked without a registry mutation. |
