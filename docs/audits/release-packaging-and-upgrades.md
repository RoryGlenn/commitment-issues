...
| [#184](https://github.com/RoryGlenn/commitment-issues/issues/184) | Addressed by the release metadata validator, reviewed changelog extraction, exact Release title/body inputs, recovery checks, and historical exceptions ledger.                                |
| [#185](https://github.com/RoryGlenn/commitment-issues/issues/185) | Addressed by the explicit runtime allowlist and closed package-import regression; close after the hosted package-manager matrix passes and the repair merges.                                  |
| [#186](https://github.com/RoryGlenn/commitment-issues/issues/186) | Addressed by exact-manifest and clean-install link validation plus canonical GitHub URLs for repository-only targets; close after hosted matrix success and merge.                             |
| [#187](https://github.com/RoryGlenn/commitment-issues/issues/187) | Addressed by time-bounded contributor grants, an effective authority matrix, and a recurring dated access review; close after the owner accepts the retention decisions and the repair merges. |

Pinned forward-upgrade evidence, the downgrade support boundary, and
prospective release metadata consistency are now defined. Fresh
registry-install evidence is still incomplete. Release retry, rollback, and
duplicate-publication behavior have a fail-closed state model and mocked
positive/negative evidence. Audit #136 must stay open until the remaining
scenarios are executed or separately dispositioned and the final
release-readiness verdict replaces this in-progress status.
...
| `npm test`                                                                                   | Passed, 797/797, including the sensitive-authority record, recurring review cadence, time-bounded access, and privacy guard.                                                                                      |
