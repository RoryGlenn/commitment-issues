...
| Packed executable contract                                                                 | The lifecycle proved the bin launched but did not explicitly record every packed mode or compare the CLI version to the packed manifest. | npm tarball metadata must contain only `scripts/cli.mjs` at `0755`; every other regular file must be `0644`. The installed bin mapping, exact Node shebang, and `--version` output must match the packed package.                                                                                                                                                                                                                                                                                                                               | Local exact-tarball verification passed; record cross-platform hosted lifecycle results.                           |
...
| `node --test test/release-integrity.test.mjs test/ci-policy.test.mjs test/metadata.test.mjs` | Passed, 52/52, including the exact workflow ancestry helper against reviewed and off-main Git histories.                                                                               |
