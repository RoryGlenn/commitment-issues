...
The remaining platform, performance, migration, and hook-manager expansion work
is explicitly tracked outside this workstream under #83, #95, #96, #97, and
#147. Those issues do not represent undispositioned findings in this audit.
...
| `init`                              | `--dry-run` or `-n`                                                              | Idempotently configures project files and provably active native hooks; preview writes nothing | Invalid input or unsafe project/config state exits 1 before mutation. Hook ownership conflicts remain non-destructive warnings. `test/init*.test.mjs`, `test/repository-shapes.test.mjs` |
...
| `doctor`                            | `--quiet`                                                                        | Repairs exact owned native wiring; quiet install mode stays exit 0                             | Interactive invalid arguments or unrecoverable wiring exit 1; quiet repair failures warn without breaking installation. `test/doctor.test.mjs`                                           |
| `commit-msg`                        | zero or one message-file positional; generated hooks provide one                 | Disabled mode is silent; enabled advisory mode reports project commitlint results              | Enabled blocking mode exits 1 for findings/setup failures. More than one positional is rejected by the bin. `test/commit-msg.test.mjs`, `test/cli.test.mjs`                              |
...
| Deferred     | Broader shell/GUI clients, large-repository performance, published-version migration, secret-inspection enforcement, and manager-specific coexistence | Requires separate matrices or product contracts                   | Tracked by #83, #95, #96, #97, and #147                                                |
