...
| Medium   | Unpinned peer bootstrap commands could resolve ESLint 10, whose dependency tree does not run at exact Node 22.11.0. Peer boundary: `package.json:109-115`; lifecycle dependency selection: `test/integration/helpers/lifecycle-fixture.mjs`; metadata regression in `test/metadata.test.mjs`. | Fixed: docs and recovery hints select ESLint 9/Prettier 3 at the floor, the fixture pins `globals@^17`, and peer metadata supports tested ESLint 9 and 10 majors.                            |
...
| Medium   | The lifecycle fixture inherited npm identity, invoked Yarn through npm tooling, and allowed Bun's runner to fetch, weakening package-manager evidence. Fixture and `select the package manager` phase: `test/integration/helpers/lifecycle-fixture.mjs`.                                      | Fixed: manager-native local-only runners, a scrubbed environment, version logging, and manager-specific recovery assertions.                                                                 |
| Medium   | A successful Git commit/push did not prove advisory hooks executed, and repeated `init` was asserted only by unit tests. A skipped/no-op hook set could therefore pass the packed lifecycle. Named init, commit, and push phase evidence: `test/integration/helpers/lifecycle-fixture.mjs`.   | Fixed: every manager runs `init` twice, and real Git operations must expose distinct pre-commit, commit-msg, and pre-push results.                                                           |
| Medium   | Scripts-disabled installation had no lifecycle evidence, so clone repair and CLI availability were conflated. Named `repair a fresh clone` phase: `test/integration/helpers/lifecycle-fixture.mjs`.                                                                                           | Fixed: the packed fixture proves absent hooks, a working local CLI, explicit doctor recovery, and later lifecycle-enabled repair.                                                            |
...
- [#100](https://github.com/RoryGlenn/commitment-issues/issues/100) adds the
  ...
  remains unsupported.
- [#175](https://github.com/RoryGlenn/commitment-issues/issues/175) restructures
  the lifecycle as named `node:test` phases over one shared disposable fixture.
  The stateful suite remains an intentionally separate packed integration gate
  rather than entering the runtime coverage denominator.
...
- Strengthened the shared packed lifecycle fixture with local-only execution,
