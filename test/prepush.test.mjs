...
test("blocking mode blocks when pushed-files diff cannot be computed", (t) => {
  const tempDir = createTempRepo();
  t.after(() => cleanupTempRepo(tempDir));
  ...
  // This also asserts pre-push invokes git diff with core.quotePath=false.
  const env = fakeGitEnv(tempDir, "-c core.quotePath=false diff");
  const result = run(
    "node",
...
test("advisory mode warns but allows when pushed-files diff cannot be computed", (t) => {
  const tempDir = createTempRepo();
  t.after(() => cleanupTempRepo(tempDir));
