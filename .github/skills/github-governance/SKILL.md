...
- The one required status context is the aggregate job **`CI Success`** in [`.github/workflows/ci.yml`](../../workflows/ci.yml): it needs DCO, quality, OS/Node tests, npm lifecycle, shell compatibility, non-npm package-manager lifecycle, migration lifecycle, and CodeQL, uses `if: always()`, and exits non-zero unless every needed job reports `success`. The Windows test and npm lifecycle results are separate parallel dependencies, and this fail-closed check rejects either one when it is skipped or otherwise incomplete.
...
- The OS/Node graph covers `{ubuntu, macos, windows} × Node {22.11.0, 24}` with `COMMITMENT_ISSUES: 0`, running the unchanged test suite once per lane and the packed npm lifecycle integration once per lane. Ubuntu's two coverage gates own the test-suite execution there rather than rerunning it first without coverage. Node 24 also verifies badge freshness. On Windows, tests and npm lifecycle are separate required jobs so they run in parallel instead of sequentially; both Node lines and both commands remain represented. `pm-lifecycle` runs pnpm 10, Yarn Classic 1.22.22, Yarn Berry 4.17.0 with `nodeLinker: node-modules`, and Bun 1.3.14 on all three OSes at Node 24 plus exact-minimum-Node lanes on Ubuntu. (`COMMITMENT_ISSUES=0` skips generated hooks — tests must strip it and legacy `HUSKY` from subprocess env; see the `testing-and-coverage` skill.)
