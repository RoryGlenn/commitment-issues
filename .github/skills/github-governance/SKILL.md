...
- The one required status context is the aggregate job **`CI Success`** in [`.github/workflows/ci.yml`](../../workflows/ci.yml): it needs DCO, quality, OS/Node tests, npm lifecycle, shell compatibility, non-npm package-manager lifecycle, migration lifecycle, and CodeQL, uses `if: always()`, and exits non-zero unless every needed job reports `success`. The two Windows Node 22.11.0 test shards, complete Windows Node 24 test lane, and npm lifecycle lanes run separately in parallel; this fail-closed check rejects the matrix when any selected test or lifecycle result is skipped or otherwise incomplete.
...
- The OS/Node evidence covers `{ubuntu, macos, windows} × Node {22.11.0, 24}` with `COMMITMENT_ISSUES: 0`. Ubuntu runs the complete suite under the two unsharded 100% coverage gates, and Node 24 also verifies badge freshness. macOS runs the complete suite unsharded on both Node lines. Windows divides the Node 22.11.0 test-file set between the complementary native shards `1/2` and `2/2`, whose union assigns every file exactly once, while Node 24 keeps one complete unsharded lane. The packed npm lifecycle integration remains represented once per OS/Node combination and runs in separate Windows jobs so it stays parallel with the test lanes. `pm-lifecycle` runs pnpm 10, Yarn Classic 1.22.22, Yarn Berry 4.17.0 with `nodeLinker: node-modules`, and Bun 1.3.14 on all three OSes at Node 24 plus exact-minimum-Node lanes on Ubuntu. (`COMMITMENT_ISSUES=0` skips generated hooks — tests must strip it and legacy `HUSKY` from subprocess env; see the `testing-and-coverage` skill.)
