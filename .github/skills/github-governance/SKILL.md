...
- The one required status context is the aggregate job **`CI Success`** in [`.github/workflows/ci.yml`](../../workflows/ci.yml): it needs DCO, quality, OS/Node tests, npm lifecycle, shell compatibility, non-npm package-manager lifecycle, migration lifecycle, and CodeQL, uses `if: always()`, and exits non-zero unless every needed job reports `success`. The Windows test-shard lanes and npm lifecycle lanes run separately in parallel, and this fail-closed check rejects the matrix when any shard or lifecycle result is skipped or otherwise incomplete.
