...
- The required status check is the aggregate job **`CI Success`** in `.github/workflows/ci.yml` (`needs: [dco, check, pm-lifecycle]`, `if: always()`), which fails unless DCO, every OS/Node matrix leg, and every package-manager lifecycle integration reports success. Skipped or otherwise incomplete required jobs fail closed too. This keeps the required-check list stable across matrix changes — don't rename it without updating the ruleset.
