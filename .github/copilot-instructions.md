...
- **Pure ESM `.mjs`, no build step, no transpile.** Node `>=22.22.1`. Uses the built-in `node --test` runner and `node:assert/strict` only — no Jest/Mocha/Vitest.
...
- **`HUSKY=0`** is set in CI and must stay stripped from test subprocess env. Reproduce CI-only failures with `HUSKY=0 node --test test/*.test.mjs`.
